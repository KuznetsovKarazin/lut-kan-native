"""
Training loop for LowRankResidualLUTKAN2Layer.

Almost identical to train_residual_kan2 but:
  - Optimizer sees (U_l1, V_l1, U_l2, V_l2) as separate parameter groups
    so we can still have per-layer learning rates (same convention).
  - best_model reconstruction carries all four factors.
  - The regularization on EFFECTIVE LUT (init + alpha * delta) is still
    correct because delta is reconstructed on each forward pass.
"""
...
@dataclass
class LowRankTrainConfig:
    lambda_1: float = 0.0
    lambda_2: float = 0.0
    # Per-layer learning rates, applied to both U and V of that layer
    lr_l1: float = 1e-4
    lr_l2: float = 5e-4
    epochs: int = 200
    batch_size: int = 128
...
@dataclass
class LowRankTrainResult:
    U_l1_best: np.ndarray
    V_l1_best: np.ndarray
    U_l2_best: np.ndarray
    V_l2_best: np.ndarray
...
def train_low_rank_kan2(
    ...
    # Per-layer learning rates: both U and V of a layer get the same lr.
    # Adam handles per-param-group lrs and momentum properly.
    optim = torch.optim.Adam([
        {"params": [model.U_l1, model.V_l1], "lr": cfg.lr_l1},
        {"params": [model.U_l2, model.V_l2], "lr": cfg.lr_l2},
    ])
    ...
    best_U1 = model.U_l1.detach().cpu().numpy().copy()
    best_V1 = model.V_l1.detach().cpu().numpy().copy()
    best_U2 = model.U_l2.detach().cpu().numpy().copy()
    best_V2 = model.V_l2.detach().cpu().numpy().copy()
    best_epoch = 0
    ...
    # Reconstruct best-model with MATCHING alpha/activation/domain/ranks.
    best_model = LowRankResidualLUTKAN2Layer(
        in_dim=model.in_dim, hidden_dim=model.hidden_dim, out_dim=model.out_dim,
        K=model.K, L=model.L, alpha=model.alpha,
        rank_l1=model.rank_l1, rank_l2=model.rank_l2,
    ...
    with torch.no_grad():
        ...
        best_model.U_l1.copy_(torch.from_numpy(best_U1))
        best_model.V_l1.copy_(torch.from_numpy(best_V1))
        best_model.U_l2.copy_(torch.from_numpy(best_U2))
        best_model.V_l2.copy_(torch.from_numpy(best_V2))
    ...
    return LowRankTrainResult(
        U_l1_best=best_U1, V_l1_best=best_V1,
        U_l2_best=best_U2, V_l2_best=best_V2,
        ...
        n_updates=n_updates,
        alpha=model.alpha,
        rank_l1=model.rank_l1,
        rank_l2=model.rank_l2,
    )
